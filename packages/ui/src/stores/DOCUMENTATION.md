...
Session folders persist in runtime-specific v2 browser keys without silently evicting older runtime namespaces. Runtime switch, page hide, app freeze, and unload synchronously flush the pending browser snapshot before lifecycle suspension or namespace replacement. A runtime switch then cancels stale old-runtime disk work and starts generation-owned disk hydration. Missing or malformed server files are not authoritative empty snapshots; disk data may replace browser state only when it carries a real revision and no newer local folder mutation occurred. The server reports missing, malformed, and valid stored state separately: a read with no file answers with an empty snapshot flagged `exists: false`, malformed or structurally invalid stored state fails the read rather than presenting itself as empty, and a valid snapshot is flagged `exists: true`. Server writes are serialized and carry monotonic revisions: a write must declare the revision it is based on, a mismatch is rejected with the current revision and state so the client reloads instead of silently losing the newer snapshot, and only state with no trustworthy revision may be repaired by a valid write. The client seeds that revision from every read, including reads whose data loses to newer browser state, so a subsequent write cannot be rejected into discarding it. File-search cache and in-flight keys include runtime plus directory and are cleared on endpoint reset.
